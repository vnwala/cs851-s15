//for jp.dxlive.com no en.
//needs /json/performer_hana -> hana_performers
...
function tryPreview () {
	if (hana_performers.list.length) {
...
function cyclePreview (w, h) {
    ...
    script.src = '/json/performer_hana';
    document.body.appendChild(script);

	//if(!user_arr) {
    if (hana_performers.free.length == 0) {
          no_perf();
	  return;
	}
	var count=hana_performers.free.length;
/*	var arr = new Array();
    ...
    if (hana_performers.free.length > 0 ){
		...
		var index = Math.floor(Math.random() * hana_performers.free.length);
		//perf = arr[index];
        perf = hana_performers.free[index];
		/*if(arr.length==1){
			perf = arr[0];
		}*/
        if (hana_performers.free.length == 1){
            perf = hana_performers.free[0];
        }
	}
