...
BB.Behaviors.billboard = function(container){
    ...
    /* fix expanded click */
    if (jQuery('.bbbiz #navigation').size()) {
    ...
    /* event page */
    if (jQuery('.tertiary-menu.business div.rss-link').size()) {
        jQuery('.tertiary-menu.business > .content').first().append(jQuery('.tertiary-menu.business div.rss-link'));
        jQuery('.tertiary-menu.business div.rss-link > a').removeClass('rss-link');
    }

    if (jQuery('.node-event-page .view-filters form').size())
    {
        var _pathname = document.location.pathname;
        var _filter = jQuery('.node-event-page .view-filters');
        _filter.find('form').attr('action', _pathname);
        if (jQuery('.with-filters.tertiary-menu').size())
        {
            jQuery('.with-filters.tertiary-menu > .content').append(_filter);
            _filter.find('input').unbind('click').unbind('submit');
            jQuery('#sidebar-second').css('margin-top', jQuery('.with-filters.tertiary-menu').height());
        }
        else
        {
            _filter.remove();
    ...
    if (jQuery('.views-exposed-widget.views-widget-filter-title').size())
    {
        jQuery('.views-exposed-widget.views-widget-filter-title').each(function()
        {
            ...
            // artists pages
            if ((/^(\/?)(artists)/.test(_pathname)) || (/^(\/?)(artist)/.test(_pathname)) || (/^(\/?)(search)/.test(_pathname)))
            {
                _filter.submit(function(){
    ...
    /* latest tweets */
    /*if (jQuery('.bb_latest_tweets').size())
    {
        var _username = jQuery('#twitter_username').val();
        var _tweets_count = jQuery("#twitter_count").val();
        if (_username) {
            jQuery('.bb_latest_tweets .twitter_view_more a').click(function(){
                jQuery('.twitter_content').hide();
                jQuery('.twitter_loading').show();

                jQuery.ajax({
                    type : "GET",
                    dataType : "json",
                    url : "http://search.twitter.com/search.json?q=from:" + _username + "&rpp=" + _tweets_count + "&callback=?",
                    success : function(data) {
                        if (data) {
                            var _results = '';
                            for (var i in data.results) {
                                _results += '<p class=\'tweet_item\'>' + data.results[i].text + '</p>';
                            }
                            jQuery(".twitter_content").html(_results);
                            jQuery('.twitter_loading').hide();
                            jQuery('.twitter_content').slideDown();
                        }
                    }
                });
            ...
            jQuery('.bb_latest_tweets .twitter_view_more a').click();
        }
    }*/
    ...
    if (jQuery('.chart_filters.top').size()) {
        jQuery('.node-type-chart-landing .full_width').first().append(jQuery('.node-type-chart-landing .chart_filters.top'));
        jQuery('.node-type-chart .full_width').first().append(jQuery('.node-type-chart .chart_filters.top'));
    }
    ...
    if (jQuery('.chart_pager_bottom .item-list ul.pager').size() && jQuery('.chart_pager_bottom.next-chart').size()) {
        next_chart = jQuery('.chart_pager_bottom.next-chart');
        jQuery('.chart_pager_bottom .item-list ul.pager').last().append('<li>'+ next_chart.html() + '</li>');
        next_chart.remove();
    }
    ...
    if (jQuery('.node-type-page .node-page .social').size()) {
        jQuery('.node-type-page .milestone .insert_social').prepend(jQuery('.node-type-page .social'));
    }
    ...
    jQuery(".module_newsletter .form-item-email input").focus(function(){
      jQuery(".module_newsletter .form-submit").addClass('focused');
    });

    jQuery(".module_newsletter .form-item-email input").blur(function(){
      jQuery(".module_newsletter .form-submit").removeClass('focused');
    });

    /* Change more-link on biz */
    if (BB.isBBBiz && jQuery('.view-id-related_articles > .more-link > a').size()) {
      jQuery('.view-id-related_articles > .more-link > a').attr('href', jQuery('.view-id-related_articles > .more-link > a').attr('href').replace('/articles', '/biz'));
    }

    /* Nav Click Tracking */
    jQuery('#block-imageblock-4 .imageblock-link').click(function() {
      _gaq.push(['_trackEvent','TopNavigation','Biz-Top-Left']);
    });

    jQuery('.bbbiz #block-imageblock-5 .imageblock-link').click(function() {
      _gaq.push(['_trackEvent','Biz','Main-Nav','Billboard-Logo']);
    });
    /* Nav Click Tracking - end */

    /* add 'active' class to the links of #block-menu-menu-secondary-menu */
    if (jQuery('#block-menu-menu-secondary-menu > div.content > ul.menu').size()) {
      var _location_href = window.location.href.split("/");
      if (_location_href.length >= 5) {
        var _content_type = _location_href[3];
        switch (_content_type) {
          case 'videos':
          case 'photos':
          case 'articles':
            jQuery('#block-menu-menu-secondary-menu > div.content > ul.menu > li > a[href="/' + _content_type + '"]').addClass('active');
            break;
          case 'artists':
            jQuery('#block-menu-menu-secondary-menu > div.content > ul.menu > li > a[href="/' + _content_type + '/top-100"]').addClass('active');
            break;
        }
      }
    }

    /* add 'active-trail' class to the links of .tertiary-menu.business */
    var _menu = jQuery('.tertiary-menu.business > div.content > ul.menu');
    if (_menu.size()) {
      var _location_href = window.location.href.split("/");
      if (_location_href.length >= 4) {
        var _current_href = '/' + _location_href[3];
        _menu.find('>li').each(function() {
          var _li = jQuery(this);
          _li.removeClass('active-trail');
          if (_li.find('>a').attr('href') == _current_href) {
            _li.addClass('active-trail');
          }
        });
      }
    }

    /* add 'active' class to the links of #block-menu-menu-3rd-bbcom-charts-menu */
    var _a = jQuery('.node-type-landing-page #content #block-menu-menu-3rd-bbcom-charts-menu.tertiary-menu .content > ul.menu li a');
    if (_a.size()) {
      _a.removeClass('active');
      _a.parent().parent().find('li.first a').addClass('active');
    ...
    var _b = jQuery('.field-name-field-profile-list .block1');
    if (_b.size()) {
      var hb = _b.height();
      var _p = _b.parent();
      var hp = _p.height();
      hp += _p.next().height();
      hp += _p.next().next().height();
      _b.css('top', Math.round((hp - hb) / 2) + 'px');
    }
        
    jQuery("a,h1").each(function(){
      jQuery(this).html(
        jQuery(this).html().replace(/&reg;/gi, '<sup>&reg;</sup>').replace(/®/gi, '<sup>&reg;</sup>').replace(/\u00AE/gi, '<sup>&reg;</sup>')
      );
    });
    
    jQuery('.chart_filters .dd').bind('touchstart', function(e) {
        e.preventDefault();
        jQuery(this).toggleClass('hover_effect');
    });

    jQuery('.chart_filters .dd a').bind('touchstart', function(e) {
        e.preventDefault();
        window.location.href = jQuery(this).attr('href');
        jQuery(this).closest('.dd').addClass('hover_effect');
    });
}
;
BB.Behaviors.milestone_lightbox = function(container){

	jQuery('#milestone_lb_wrapper .close').click(function(event){
		event.preventDefault();
		closeLB();
	});

	jQuery("#milestone_lb").load(function() {
		setTimeout(function(){
    if (!container.hasClass('not_auto')) {
      container.css({
       height: "auto"
      });
      container.height(container.contents().height());
      var window_height = window.innerHeight || document.documentElement.clientHeight;    
      if (typeof window.innerHeight === 'undefined') {
        window_height = window_height - 30;
      }
      if (container.height() > window_height) {
        var new_height = window_height - jQuery("#bb").offset().top;                  
        if (typeof window.innerHeight === 'undefined') {
          var iframe_height = new_height + 20;
          jQuery('#milestone_lb').css('height', iframe_height + 'px'); 
          new_height = new_height - 50;                             
          jQuery('#milestone_lb_wrapper').css('overflow-y', 'scroll').css('height', new_height + 'px');
        } else {
          jQuery('#milestone_lb_wrapper').css('overflow-y', 'scroll').css('height', new_height + 'px');
        }
      }
	...
	function openForm() {
		openLB('/milestone/thankyou');
	}

  function openLB(href) {
    if (href.length > 0) {
      container.attr("src", href);
      jQuery('#milestone_lb_m').fadeIn(250);
      jQuery('#milestone_lb_wrapper').show();
	...
	function closeLB() {
		jQuery("#milestone_lb_m").fadeOut(250, function(){
      jQuery('#milestone_lb_wrapper').hide().attr('style', '');
      container.attr("src", '');
		});
		container.fadeOut(250);
	}
  
  jQuery(".milestone .left_banner a").click(function(e){
    e.preventDefault();
    openLB("/milestone/enter");    
  });

	jQuery("#milestone_lb_m").on("click",function(event){
		closeLB();
	});

	jQuery(".milestone .listing .article .v a").live("click",function(e){
    e.preventDefault();
    var link = jQuery(this);
		var choice = jQuery(this).attr('rel');

    jQuery('#milestone_lb_m').fadeIn();
    jQuery('#milestone_lb_m').css('cursor', 'progress');

		if (choice.length > 0) {
		  jQuery("#radioDefault_2").val(choice);
      jQuery('form.wufoo').submit();
      setTimeout(function() {
        jQuery('#milestone_lb_m').hide();
        jQuery('#milestone_lb_m').css('cursor', 'auto');
        if (link.hasClass('new')) {
          openLB('/milestone/thankyou_vote');
        } else {
          if (link.hasClass('win')) {
            if (typeof Drupal.settings.win_sonic_url !== 'undefined') {
              window.location = Drupal.settings.win_sonic_url;
            } else {
              window.location = '/milestone/sonic';
            }
          } else {
            openLB('/milestone/thankyou');
          }
  ...
  jQuery(".milestone .milestone_popup").click(function(e){
    e.preventDefault();
    var url = '/milestone/enter';
    openLB(url);
 });

 var hash = window.location.hash.substring(1);
  if (hash == "win") {
    jQuery("#milestone_lb").height(560);
    var url = '/milestone/win_success';
    openLB(url);
  }
};

jQuery(document).ready(function(){
  if (jQuery('.field-name-field-milestone-nominees .twit a').length > 0) {
    var max_height = 0;
    jQuery('.field-name-field-milestone-nominees .twit a').each(function(){
...
function setOpenDrawerSubmenu(container) {
    ...
    container.find('> a').click(function(event){
        ...
        if (!container.hasClass('open')) {
            ...
            drawer.find('[placeholder]').addPlaceholder({'checkafill': true});
        } else {
            drawer.slideUp();
...
BB.Behaviors.fake_header = function(container) {
  ...
  var $next_element = container.next(),
      ...
      init = Math.round(top_pos - h);

  container.width(container.parent().width());
  jQuery(window).scrollTop(init);
  ...
  jQuery(window).on("scroll", function() {
    if (container.is(':visible')) {
      ...
      if (sT < init || sT > init + h) {
        container.fadeOut(250);
        clearInterval(fakeHeaderInterval);
      }
    }
  });

  var fakeHeaderInterval = setInterval(function() {
    if ($next_element.offset().top != top_pos) {
      top_pos = $next_element.offset().top;
      init = Math.round(top_pos - h);
      jQuery(window).scrollTop(init);
      //clearInterval(fakeHeaderInterval);
...
BB.Behaviors.gallery = function(container) {
  ...
  var $ph = container.find(".photos"),
      ...
      track_page = false,
      image_data,
      bbcom = jQuery('.node-photo-gallery').hasClass('bbcom');
  ...
  function load_data() {
    var currentFid = getQueryVariable('i');
    jQuery.getJSON(feed_url+'?gallery_id='+feed_id+'&i='+currentFid+(bbcom ? '&site=bbcom' : ''), function(data) {
      if (data.length > 0) {
        image_data = data;
        ...
        jQuery.each(data, function(i,v){
          // insert advert
          if (!bbcom && ad_every && i > 0 && i % ad_every == 0 && i != (max - 1)) {
              lis.push(insertAd(i));
              max++;
              j++;
              ads++;
          ...
          html += '<li data-num="'+i+'" data-fid='+v.fid+embed_class+'>';
          if (v.html != '') {
            html += '<div class="photo-embeded">';
          } else {
            html += '<div class="photo">';
          }
          if (v.src != '') {
            html += '<img '+a_src+'="'+v.src+'" alt="'+v.alt+'" />';
            if (bbcom) {
              html += v.pinit;
              html += '<i>'+v.credit+'</i>';
            }
          ...
          html += '</div>';
          if (!bbcom) {
            html += '<div class="photo_caption">'+v.caption+v.credit+v.pinit+'</div>';
          }
          html += '</li>';
          lis.push(html);
          j++;
        ...
        if (bbcom) {
          ...
          currently_visible.addClass('visible');
        }
  ...
  function countAdsBeforeCurrentPos() {
    var count = 0;
    if (ad_every) {
      count = Math.floor( (currentSet - 1) / (ad_every + 1) );
    }
    return count;
  }
  ...
  function nextSlide(direction) {
    if (!sliderMoving) {
      ...
      // BB-3887 - http://stackoverflow.com/a/7513356/938089
      var iframe = $ph.children().eq(previousPosition).find('iframe');
      if (iframe.length > 0 && iframe.attr('src') && iframe.attr('src').match(/youtube/g)) {
        iframe.get(0).contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
      }

      // for gumgum
      if (bbcom) {
        var previously_visible = $ph.children().eq(previousPosition);
        previously_visible.removeClass('visible');
        currently_visible.addClass('visible');
      }

      // check for advert
      var html = '<div id="dfp-ad-m1_iframe" class="dfp-tag-wrapper"><script type="text/javascript">googletag.cmd.push(function() { googletag.display("dfp-ad-m1_iframe"); }); </script></div></li>';
      if (currently_visible.hasClass("ad")) {
        if (!bbcom) {
          $f.addClass("ad");
          if (!currently_visible.hasClass("last")) {
            jQuery('.photos li.ad').not('.last').empty();
            googletag.pubads().refresh();
            currently_visible.html(html);
          }
        }
      } else {
        $f.removeClass("ad");
      ...
      if (bbcom) {
        if (currently_visible.hasClass('last')) {
          jQuery('.nextPrev.top li, #photo-social-wrapper, .comments').fadeOut();
          jQuery('.nextPrev.on-image').addClass('hidden');
        } else {
          ...
          if (slots) {
            for (var i in slots) {
              ...
              domid = slotid.getDomId();
                if ('dfp-ad-m1_iframe' == domid) {
                  slot = slots[i];
                  break;
                }
            }
          ...
          /*
          if (jQuery("#grv-personalization-18").length > 0) {
            jQuery("#grv-personalization-18 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          if (jQuery("#grv-personalization-19").length > 0) {
            jQuery("#grv-personalization-19 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          if (jQuery("#grv-personalization-20").length > 0) {
            jQuery("#grv-personalization-20 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          if (jQuery("#grv-personalization-56 iframe").length > 0 && jQuery("#grv-personalization-56").is(':visible')) {
            jQuery("#grv-personalization-56 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          if (jQuery("#grv-personalization-13 iframe").length > 0 && jQuery("#grv-personalization-13").is(':visible')) {
            jQuery("#grv-personalization-13 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          if (jQuery("#grv-personalization-42 iframe").length > 0 && jQuery("#grv-personalization-42").is(':visible')) {
            jQuery("#grv-personalization-42 iframe").each(function(){
               jQuery(this).attr('src', jQuery(this).attr('src'));
            });
          }
          */
      ...
      $ph.animate({left: currentSliderPos}, speed, function () {
        if (reset) {
  ...
  function loadImages() {
    ...
    $ph.height($ph.children().eq(currentSet).height());
  }
  ...
  function update_numerator() {
    if (currentSet == max) {
      ...
      if (bbcom) {
        ...
        var ads_count = countAdsBeforeCurrentPos();
        $n.fadeIn().text((currentSet - ads_count) + " / " + (max-1-ads));
      }
    }
  }
  ...
  function set_caption() {
    var currently_visible = $ph.children().eq(currentSet);
    var container = jQuery('.photo-caption');
  ...
  function set_url() {
    ...
    if (track_page) {
      ...
      currently_visible.find('.comscore').remove();
      currently_visible.prepend('<div class="comscore"><img src="http://b.scorecardresearch.com/p?c1=2&c2=7395269&cv=2.0&cj=1" /></div>');
    }
    track_page = true;
  }

  function init_next_gallery() {
    var hitEvent = 'ontouchstart' in document.documentElement ? 'touchstart' : 'click';

    jQuery('a.previous-galleries-block-next').bind(hitEvent, function(){
      window.location.href = jQuery("a.previous-galleries-block-next").attr('href');
    });
  }
  ...
  function init_replay() {
    ...
    jQuery('.previous-galleries-block-replay').bind(hitEvent, function(){
      nextSlide('replay');
      jQuery('.nextPrev.top li, #photo-social-wrapper, .comments').fadeIn();
      jQuery('.nextPrev.on-image').removeClass('hidden');
    });
  }
...
;
BB.Behaviors.infographic = function(container) {
  jQuery('.infographic_modal', container).once().each(function(){
...
BB.Behaviors.jcarousel = function(container){
    ...
    var $total = container.find('li').show().size();
...
BB.Behaviors.breaking_news_bar = function(container) {
    ...
    container.find("a .icon.fb").parent().unbind("click").click(function(event){
      event.preventDefault();
      socialopen(jQuery(this).attr("href"));
      return false;
    });
...
BB.Behaviors.sticky_social_links = function(container) {
    ...
    var padding = 20,
        ...
        upperlimit = init_offset.top - padding,
    ...
    jQuery(window).scroll(function(event){
        ...
        if (scrollTopNum > upperlimit && scrollTopNum < lowerlimit) {
            clearInterval(stickySocialLinksInterval);
            container.css({
                position: "fixed",
                top: padding,
    ...
    var stickySocialLinksInterval = setInterval(function() {
        //if (container.offset().top != init_offset.top) {
            init_offset = container.offset(),
            upperlimit = init_offset.top - padding,
            lowerlimit = ($article_content) ? $article_content.offset().top + $article_content.outerHeight(true) - container.outerHeight(true) : 99999;
        //}
        ...
        //if(jQuery.browser.msie) {
            jQuery('.social_vert ul.with_buttons').hover(
                function() {
                    ...
                    jQuery('li .g-plusone-wrapper iframe, li .tweetbutton iframe', this).css({
                        'overflow': 'visible',
                        'width': '58px'
                    });                    
                },
...
// Social horizontal plugin
function set_social_hover_actions() {

  var social_li = jQuery('.social:not(.social_vert) ul.with_buttons li, .social-milestone ul li');

  social_li.mouseenter(function() {
    if (jQuery(this).parents('.social-milestone').size()) {
      var width = '100px';
    } else {
      var width = 'auto';
    }
  ...
  /*if (jQuery.browser.msie) {
    jQuery('.social:not(.social_vert) ul.with_buttons li iframe, .social-milestone ul li iframe').live('hover',function() {
      jQuery(this).parents('.social:not(.social_vert) ul.with_buttons li > div, .social-milestone ul li > div').css({
        'border': '1px solid #e6e6e6',
        'width': 'auto',
        'height': 'auto',
        'left': '-1px',
      ...
      var obj = jQuery(this).parents('li').first().find('> div');

      var hidePopupInterval = setInterval(function() {
        obj.css({
          'width': '0px',
          'height': '0px',
          'border': '0px',
...
BB.Behaviors.social_milestone = function(container) {
  jQuery(function($) {
    if ($.browser.msie) {
      $('.social-milestone li iframe').live('hover', function() {
        $(this).parents('li').addClass('hover');
      });
      $('.social-milestone li').live('mouseleave', function() {
        $(this).removeClass('hover');
      });
    }
  });
...
BB.Behaviors.chart_name_search = function(container) {
    var css_class = container.data("dropdown-class") || "";
    container.select2({
    ...
    jQuery('.search_results .search_refine').change(function(){
