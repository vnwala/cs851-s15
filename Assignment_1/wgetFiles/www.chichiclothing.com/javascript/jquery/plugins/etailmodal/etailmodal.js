 ...
 */
(function ($) {
	$.etailModal = function(options) {
		return $.etailModal.modal.init(options);
	};
	...
	$.modal.close = function () {
		return $.etailModal.modal.close(true);
	};

	$.etailModal.close = function () {
		return $.etailModal.modal.close(true);
	};

	$.fn.etailModal = function (options) {
		options = $.extend({}, {
			type: 'inline',
			inline: $(this).html()
		}, options);
	...
	$.etailModal.modal = {
		...
		init: function(options) {
			...
			this.options = $.extend({}, $.etailModal.defaults, options);
		...
		displayModal: function(data)
		{
			...
			if(data.indexOf('ModalTitle')>0 && data.indexOf('ModalContent')>0){
				...
				modalContent += '<div id="ModalContentContainer"><div class="ModalContent">'+data+ '</div></div>';
			}
...
function ModalBoxInline(title, content, width, withCloseButton){
	...
	if(withCloseButton){
		...
		var str = '<div class="ModalTitle">'+title+'</div><div class="ModalContent">'+$(content).html()+ '</div><div class="ModalButtonRow"><input type="button" class="CloseButton FormButton" value="Close Window" onclick="$.etailModal.close();" /></div>';
	}
	$.etailModal({ 'data': str, 'width':width });
}