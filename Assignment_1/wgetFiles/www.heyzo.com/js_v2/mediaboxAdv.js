...
(function() {
	...
	// DOM elements
	overlay, center, media, bottom, captionSplit, title,name, caption, number, prevLink, nextLink,
	// Mediabox specific vars
	URL, WH, WHL, elrel, elname,mediaWidth, mediaHeight, mediaType = "none", mediaSplit, mediaId = "mediaBox", margin, marginBottom;
	...
	window.addEvent("domready", function() {
		...
		bottom = new Element("div", {id: "mbBottom"}).inject(center, "inside").adopt(
			...
			name = new Element("div", {id: "mbName"}),
...
//			NonverBlaster
				playerpath: '/assets/json2.swf',	// Path to NonverBlaster.swf
		...
		*/
		mediabox: function(_options, linkMapper, linksFilter) {
			linkMapper = linkMapper || function(el) {
				elrel = el.rel.split(/[\[\]]/);
				elrel = elrel[1];
				return [el.get('href'), el.name, elrel];	// thanks to Dušan Medlín for figuring out the URL bug!
			};
...
// FLV, MP4
				...
				preload = new Swiff(''+options.playerpath+'?file='+URL+'&allowSmoothing=true&autoPlay='+options.autoplay+'&provider=http&buffer=2&showTimecode='+options.showTimecode+'&loop='+options.medialoop+'&controlColor='+options.controlColor+'&controlBackColor='+options.controlBackColor+'&playerBackColor='+options.playerBackColor+'&defaultVolume='+options.volume+'&scaleIfFullScreen=true&showScalingButton=true&crop=false&skin=/assets/heyzo_l.zip&autostart=true', {
					id: 'mbVideo',
					width: mediaWidth,
					height: mediaHeight,
...
//		if (Browser.Platform.ios && (mediaType == "obj" || mediaType == "qt" || mediaType == "html")) mediaType = "ios";
		...
		if (mediaType == "img"){
			...
			if (options.imgBackground) {
				...
				// ギャラリーのダウンロードを可能にするためにコメントアウト if (options.clickBlock) preload.addEvent('mousedown', function(e){ e.stop(); }).addEvent('contextmenu', function(e){ e.stop(); });
